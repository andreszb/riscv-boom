  ...
  with freechips.rocketchip.rocket.constants.ScalarOpConstants
{
  ...
  // erlingrj: Does this instruction cast a C-shadow (control-flow-shadow). TODO: Why cannot jal miss-speculate?
  def has_c_shadow: Bool    = is_br_or_jmp && !is_jal

  /*
  def has_e_shadow: Bool = is division?
  def has_m_shadow: Bool = is load
  def has_
   */
