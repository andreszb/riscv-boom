...
// HACK BY erlingrj to do CLI parameter setting
...
/**
 * Following mixins are created by erlingrj to do benchmarking on various Shadow Buffer and Release Queue sizes
 */

class WithSmallCustomBooms extends Config((site, here, up) => {
  case BoomTilesKey => up(BoomTilesKey, site) map { b => b.copy(
    core = b.core.copy(
...
class WithMediumCustomBooms extends Config((site, here, up) => {
  case BoomTilesKey => up(BoomTilesKey, site) map { b => b.copy(
    core = b.core.copy(
      numRqEntries = CustomParams.NUM_RQ_ENTRIES,
      numSbEntries = CustomParams.NUM_SB_ENTRIES,
