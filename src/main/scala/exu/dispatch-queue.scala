                                 ...
                                 )(implicit p: Parameters) extends DispatchQueue(params.numEntries, params.deqWidth, params.enqWidth, params.qName)
{
  ...
  val heads_out_brmask = Wire(Vec(deqWidth, UInt(brTagSz.W)))
  ...
  for(i <- 0 until enqWidth) {
    ...
    s2_sram_read_row(i) := row
    s2_sram_read_idx(i) := col
    s2_sram_read_idx_map(col) := i.U
    s2_sram_read_uop(i) := sram_fifo(i)(row)
    s2_sram_read_valids(i) := valids(i)(row)
  }
  ...
  // Shift and refill the head regs.
  ...
  for (i <- 0 until deqWidth) {
    ...
    when(heads_will_be_valid(i)) {
      ...
      assert(!heads_killed(i), "[dis-q] shifting killed head")

    }
  }

  // Refill the remaining head slots
  for (i <- 0 until deqWidth) {
    when (i.U < refills_needed) {
      val heads_idx = deqWidth.U - refills_needed + i.U
      val col = s2_sram_read_idx_map(i)
      val row = Wire(UInt(qAddrSz.W))
      row := s2_sram_read_row(col)
      when (!io.flush) {
        val uop = Mux(bypass_valids(i), bypass_uops(i), s2_sram_read_uop(col))
        val valid = Mux(bypass_valids(i), true.B, valids(row)(col))
        ...
        deqs(i) := Mux(bypass_valids(i), false.B, valids(row)(col))
